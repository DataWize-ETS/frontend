...
function Form() {
  ...
  function loadId(id) {
    api.get(`/improvement/${id}`)
    .then((response) => {
        setName(response.data.signature)
        setEmail(response.data.email)
        setDepartmentsValue(response.data.departmentId)
        setRegion(response.data.region)
        setDescription(response.data.description)
        setSummary(response.data.summary)
        setDueDate(response.data.dueDate)
        setCipPex(response.data.cipPex)
        setcurrentOrganization(response.data.region) 
        setSourceTypesValue(response.data.sourceTypeId)
        setIdeaSourcesValue(response.data.ideaSourceId)
        setCurrency(response.data.currency)
        setPriority(response.data.priorityId)
        setMajorBenefitsValue(response.data.majorBenefitsid)
        setAffectEntities(response.data.affectEntities)
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      if(id) {
        ...
        axios
        .put(`improvement/${id}`, bodyParameters, config)
        .then((data) => {
        ...
        axios
        .post("/improvement/", bodyParameters, config)
        .then((data) => {
      ...
      <form action="" method="post" onSubmit={(e) => handleSubmit(e)}>
          ...
          <div className="col-2">
            <FormControl id="first-name">
              <Select
                ...
                id="department"
                name="department"
                onChange={(e) => setDepartmentsValue(e.target.value)}
              ...
              <div className="col-6">
                <FormControl variant="floating" id="first-name">
                  <Input
                    ...
                    id="cipPex"
                    name="cipPex"
                    onChange={(e) => setCipPex(e.target.value)}
                    value={cipPex}
                  />
          ...
          <div className="col-3">
            <FormControl variant="floating" id="first-name">
              <Select
                ...
                id="sourceType"
                name="sourceType"
                onChange={(e) => setSourceTypesValue(e.target.value)}
                value={sourceTypesValue} /*multiple={true}*/
              >
                {sourceTypes.map((sourcetype, i) => {
                  return (
                    <option key={i} value={sourcetype.sourceTypeId}>
                      {sourcetype.name}
                    </option>
                  );
                })}
          ...
          <div className="col-2">
            <FormControl id="first-name">
              <Select
                ...
                id="ideaSource"
                name="ideaSource"
                onChange={(e) => setIdeaSourcesValue(e.target.value)}
                value={ideaSourcesValue}
              >
                {ideaSources.map((ideasource, i) => {
                  return (
                    <option key={i} value={ideasource.ideaSourceId}>
                      {ideasource.name}
                    </option>
                  );
                })}
        ...
        <div className="row">
          <div className="col-2">
            <FormControl variant="floating" id="first-name">
              <Select
                placeholder="Major Benefits"
                h={14}
                className="input"
                id="majorBenefits"
                name="majorBenefits"
                onChange={(e) => setMajorBenefitsValue(e.target.value)}
                value={majorbenefitsValue}
              >
                {majorbenefits.map((majorbenefit, i) => {
                  return (
                    <option key={i} value={majorbenefit.majorBenefitsid}>
                      {majorbenefit.name}
                    </option>
                  );
                })}
          ...
          <div className="col-3">
            <FormControl id="first-name" variant="floating">
              <Input
                ...
                id="affectEntities"
                name="affectEntities"
                onChange={(e) => setAffectEntities(e.target.value)}
                value={affectEntities}
              />
        ...
        <button type="submit">{ !id ? 'Send Improvement' : 'Save and Approve'}</button>
      </form>
    </ChakraProvider>
  );
}
